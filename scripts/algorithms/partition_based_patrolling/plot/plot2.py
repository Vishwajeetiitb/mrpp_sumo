...
for idx,bot in enumerate(no_of_bots):
    for m, tag in enumerate(deploy_tag):
        ...
        fig.add_trace(go.Scatter(x=stamps, y=val,mode='lines',marker=dict(color=color_list[m]),showlegend=(True if idx==0 else False),name='on_{}'.format(tag),legendgroup=m),row=row_id,col=col_id)

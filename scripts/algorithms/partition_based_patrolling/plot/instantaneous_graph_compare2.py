...
for m, no_agents in enumerate(no_agents_list):
    ...
    for idx, device_range in enumerate(device_ranges):
        ...
        fig.add_trace(go.Scatter(x=stamps, y=val, mode='lines', marker=dict(color=color_list[m]), legendgroup=m+1, name='{} Agents'.format(
            no_agents), showlegend=(True if idx == 0 else False)), row=int(idx/col_size)+1, col=idx % col_size+1)
        ...
        if scater_nodes_algo_index != -1 and scater_nodes_algo_index == m:
            ...
            fig.add_trace(go.Scattergl(x=np.repeat(stamps, idle.shape[1]), y=idle.flatten(),
                ...
                line_width=0.5), legendgroup=100, showlegend=(True if idx == 0 else False), name='{} Agents'.format(no_agents)), row=int(idx/col_size)+1, col=idx % col_size+1)

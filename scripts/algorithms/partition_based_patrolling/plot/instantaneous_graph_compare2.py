...
fig = make_subplots(rows=row_size, cols=col_size,subplot_titles=device_names,vertical_spacing=0.1,horizontal_spacing=0.05)
...
for m,no_agents in enumerate(no_agents_list):
    ...
    for idx,device_range in enumerate(device_ranges):
        ...
        fig.add_trace(go.Scatter(x=stamps, y=val,mode='lines',marker=dict(color=color_list[m]),legendgroup=m+1,name='{} Agents'.format(no_agents),showlegend=(True if idx==0 else False)),row=int(idx/col_size)+1,col=idx%col_size+1)

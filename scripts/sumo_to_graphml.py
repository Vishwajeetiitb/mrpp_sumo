...
def graph_wrapper(graph_name ='grid_5_5'):
    ...
    net_file = sumo_folder + '/{}.net.xml'.format(graph_name)
    net = sumolib.net.readNet(net_file)
    G = nx.MultiDiGraph()
      
    graphml_file = graphml_folder + '/{}.graphml'.format(graph_name)

    sumo_to_graphml(node_file, edge_file, net_file,graphml_file)
...
def sumo_to_graphml(node_file, edge_file,net_file, graphml_file):
    ...
    edge_parse = et.parse(net_file)
    edge_root = edge_parse.getroot()
    for e in edge_root.iter('edge'):
        print(e[0].attrib['shape'])
